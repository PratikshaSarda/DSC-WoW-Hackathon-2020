...
def event2():
    ...
    def save():
        con = sqlite3.connect("my_db1")
        query = "insert into criminal_data values ("+txt1.get()+",'"+str(txt2.get())+"','"+str(txt6.get())+"','"+str(txt8.get())+"','"+str(txt9.get())+"','"+str(txt7.get())+"',"+","+txt3.get()+",'"+str(var.get())+"','"+str(txt4.get())+"','"+str(txt5.get())+"','"+str(txt10.get())+"','"+str(txt11.get())+"')"
        con.execute(query)
        con.commit()
        con.close()
    btn1 = Button(temp, width="13", height="1", bd="5", activebackground="green", text="SAVE",command=save)
